...
async def cmd_start(message: types.Message):
    ...
    if (message.text.lower() == '/start') or (message.text.lower() == '/help'):
        ...
        helpmessage=' \nЗадача управления ,боксами сводится к просмотру списка боксов, которыми \
        Вам разрешено управлять и проверке состояния дверей бокса \
        \nДля управления замком дверей \nбокса предусмотрены кнопки \
        заблокировать и разблокировать '
        await message.answer(startmessage+helpmessage, reply_markup='') 
        await message.answer('По вопросам работы бота просьба обращаться @yurasoft1972', reply_markup=keyboard)
        #print(checkmessage)
