...
def updatestate(statestr: str) -> None:
#    print(menulevelstr)
    sqlline=f"update state set state = '{statestr}' where id = 1"
#    print(sqlline)
    cursor.execute(sqlline)
    conn.commit()

def getstate() -> str:
    sqlline=f"SELECT state FROM 'state' where id = 1"
    cursor.execute(sqlline)
...
def updateselectedboxbox(box: str) -> None:
    sqlline=f"update 'selectbox' set 'box' = '{box}'"
    cursor.execute(sqlline)
    conn.commit()

def getlistboxforuser(user:str)  -> str:
    cursor.execute(f"select boxes FROM users WHERE user='{user}'")
    result = cursor.fetchone()
    conn.commit()
    return result

def getlistusersuser()  -> str:
...
def getallboxes()  -> str:
    cursor.execute(f"select box FROM boxes")
    result = cursor.fetchall()
    conn.commit()
    return result    
check_db_exists()
